<template>
  <div>
    <!-- 分割线-->
    <enter-stock-not-check-table-title class="margin-bottom-20"/>
    <el-table
        ref="multipleTable"
      ...
      <el-table-column prop="drugValidity" label="药品有效期"/>
      <el-table-column prop="totalBuyPrice" label="采购总金额(元)"/>
      <el-table-column prop="totalSalePrice" label="零售总金额(元)"/>
    </el-table>
...
<script>
import EnterStockNotCheckTableTitle
  from "@/views/drug_manage/enter_stock_manage/enter_stock_not_check/children/EnterStockNotCheckTableTitle";

export default {
  name: "EnterStockNotCheckTable",
  components: {
    EnterStockNotCheckTableTitle
  },
  data() {
    return {
      //每页数据
  ...
  methods: {
    ...
    toOutStockPassCheck() {
      this.$router.push('/home/outStockPassCheck/10')
    },
